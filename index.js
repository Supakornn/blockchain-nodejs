...
class HexCoin {
    ...
    createNewChain = (req, res) => {
        const block = GlobalChain.create(req.body.id, req.body.name, req.body.genesis,
        )
        res.status(200).json({ message: 'Chain created!', data: GlobalChain });
    }

    appendNewChild = (req, res) => {
        const block = new Block(this.chain.length, req.body.timestamp, req.body.transaction)
